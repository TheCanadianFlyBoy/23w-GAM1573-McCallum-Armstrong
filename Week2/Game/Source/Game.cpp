    ...
    : GameCore( fwCore )
{
    ...
    //Static body
    b2PolygonShape PlanarShape;
    PlanarShape.SetAsBox(100.f, 1.f);
    //Body def
    b2BodyDef PlanarDef = b2BodyDef();
    PlanarDef.position.Set(0.f, -5.f);
    PlanarDef.type = b2_staticBody;
    //Fixture def
    b2FixtureDef PlanarFixture;
    PlanarFixture.shape = &PlanarShape;
    PlanarFixture.friction = 0.5f;
    PlanarFixture.density = 1.f;

    b2Body* PlanarBody = m_pWorld->CreateBody(&PlanarDef);
    PlanarBody->CreateFixture(&PlanarFixture);
    ...
    {
        pObject->CreateBody(m_pWorld, vec2(0.6f,0.6f));
    }
}
